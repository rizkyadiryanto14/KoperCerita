...
(function (factory) {
	...
	Mapael.prototype = {
		 ...
		 *
		 * This method can be call directly by backend:
		 *     $(".mapcontainer").data("mapael").destroy();
		 *
		 * This method is also automatically called if the backend try to call mapael
		...
		initResponsiveSize: function () {
			...
			self.onResizeEvent = function () {
				...
				// setTimeout to wait for the backend to finish its resizing
		...
		/*
		 * Extend the backend option with the default one
		 * @param options the backend options
		 * @return new options object
		 */
		extendDefaultOptions: function (options) {

			// Extend default options with backend options
			...
			$.each(self.customEventHandlers, function (eventName) {
				...
				self.$container.off(fullEventName).on(fullEventName, "[data-id]", function (e) {
					...
					if (!self.panning &&
						...
						// Run callback provided by backend
		...
		initZoom: function (mapWidth, mapHeight, zoomOptions) {
			...
			// When the backend drag the map, prevent to move the clicked element instead of dragging the map (behaviour seen with Firefox)
		...
		onZoomEvent: function (e, zoomOptions) {
			...
			if (zoomOptions.area !== undefined) {
				...
				// Get backend defined zoom level
		...
		/*
		 * Show some element in range defined by backend
		 * Triggered by backend $(".mapcontainer").trigger("showElementsInRange", [opt]);
		 *
		...
		showElemByRange: function (ranges, elems, hiddenOpacity, animDuration) {
			...
			$.each(ranges, function (valueIndex) {
				...
				// Check if backend defined at least a min or max value
		...
		/*
		 * Set backend defined handlers for events on areas and plots
		 * @param id the id of the element
		 * @param type the type of the element (area, plot, link)
		...
		drawLegend: function (legendOptions, legendType, elems, scale, legendIndex) {
			...
			for (i = 0; i < sliceOptions.length; ++i) {
				...
				if (sliceOptions[i].display === undefined || sliceOptions[i].display === true) {
					...
					// Hide map elements when the backend clicks on a legend item
					if (legendOptions.hideElemsOnClick.enabled) {
						// Hide/show elements when backend clicks on a legend element
		...
		/*
		 * Allow to hide elements of the map when the backend clicks on a related legend item
		...
		animateViewBox: function (targetX, targetY, targetW, targetH, duration, easingFunction) {
			...
			var computeNextStep = function () {
				...
				// This is the case when the backend is triggering the zoom fast (e.g. with a big mousewheel run)
				...
				if (elapsed < durationWithMargin) {
					...
					//
					// A change of ViewBox target between steps means the backend is triggering
					...
					// the backend will see a delay at beginning the ratio will stays at 0 for some frames
					//
					...
					if (zoomDir === 'in' && (w > self.currentViewBox.w || w < targetW)) {
						...
						// Otherwise, the backend would see the camera going back and forth
